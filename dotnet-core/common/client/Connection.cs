...
namespace mcnntp.common.client
{
    ...
    internal class Connection
    {
        ...
        internal Task<bool> Send(string format, params object[] args)
        {
        ...
        /// <param name="args">The argument applied as a format string to <paramref name="format"/> to create the data to send to the client</param>
        /// <returns>A value indicating whether or not the transmission was successful</returns>
        private Task<bool> SendCompressed(string format, params object[] args)
        {
            return this.SendInternal(string.Format(CultureInfo.InvariantCulture, format, args), true);
        }

        private async Task<bool> SendInternal(string data, bool compressedIfPossible)
        {
